...
void ItemFactory::createNitems(const sf::Texture& texture, std::vector<Nitem>& nitems, int itemWidth, int itemHeight, int numNitems, const std::vector<Platform*>& platforms, float scale) {
    std::srand(static_cast<unsigned int>(std::time(nullptr)));

    for (int i = 0; i < numNitems; ++i) {
        int x = (i * itemWidth) % texture.getSize().x;
        int y = ((i * itemWidth) / texture.getSize().x) * itemHeight;
        ...
        nitems.emplace_back(texture, rect, sf::Vector2f(posX, posY), scale); // Pasa el factor de escala aquí
    }
}
